...
export class Simulation extends Scene {
	...
	display(context, program_state) {
		...
		if (program_state.animate) this.simulate(program_state.animation_delta_time)
...
export class Collision_Demo extends Simulation {
	...
	update_state(dt, num_bodies = 40) {
		...
		while (this.bodies.length < num_bodies)
			this.bodies.push(
				new Body(this.data.random_shape(), undefined, vec3(1, 5, 1)).emplace(
		...
		for (let a of this.bodies) {
			...
			for (let b of this.bodies) {
				...
				if (!a.check_if_colliding(b, collider)) continue
