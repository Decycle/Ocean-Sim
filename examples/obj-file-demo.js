...
export class Shape_From_File extends Shape {
	...
	parse_into_mesh(data) {
		...
		}
		{
			...
			// const lower_bound = vec3(verts[0], verts[1], verts[2])
			// const upper_bound = vec3(verts[0], verts[1], verts[2])
			for (var j = 0; j < verts.length / 3; j++) {
				// if (verts[3 * j] < lower_bound[0]) lower_bound[0] = verts[3 * j]
				// if (verts[3 * j + 1] < lower_bound[1]) lower_bound[1] = verts[3 * j + 1]
				// if (verts[3 * j + 2] < lower_bound[2]) lower_bound[2] = verts[3 * j + 2]
				// if (verts[3 * j] > upper_bound[0]) upper_bound[0] = verts[3 * j]
			...
			// console.log('x', upper_bound[0] - lower_bound[0])
			// console.log('y', upper_bound[1] - lower_bound[1])
			// console.log('z', upper_bound[2] - lower_bound[2])
		}
