  ...
  parse_into_mesh(data) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (VERTEX_RE.test(line))
        ...
        var quad = false
        for (
          var j = 0, eleLen = elements.length;
          j < eleLen;
          j++
        ) {
          if (j === 3 && !quad) {
          ...
          if (elements[j] in unpacked.hashindices)
            ...
            unpacked.verts.push(
              +verts[(vertex[0] - 1) * 3 + 0]
            )
            unpacked.verts.push(
              +verts[(vertex[0] - 1) * 3 + 1]
            )
            unpacked.verts.push(
              +verts[(vertex[0] - 1) * 3 + 2]
            )

            if (textures.length) {
              unpacked.textures.push(
                +textures[
                  (vertex[1] - 1 || vertex[0]) * 2 + 0
                ]
              )
              unpacked.textures.push(
                +textures[
                  (vertex[1] - 1 || vertex[0]) * 2 + 1
                ]
              )
            }
            ...
            unpacked.norms.push(
              +vertNormals[
                (vertex[2] - 1 || vertex[0]) * 3 + 0
              ]
            )
            unpacked.norms.push(
              +vertNormals[
                (vertex[2] - 1 || vertex[0]) * 3 + 1
              ]
            )
            unpacked.norms.push(
              +vertNormals[
                (vertex[2] - 1 || vertex[0]) * 3 + 2
              ]
            )
...
export class Obj_File_Demo extends Scene {
  ...
  display(context, program_state) {
    ...
    program_state.lights = [
      new Light(
        Mat4.rotation(t / 300, 1, 0, 0).times(
          vec4(3, 2, 10, 1)
        ),
        color(1, 0.7, 0.7, 1),
        100000
