...
float de(vec3 p) {
    ...
    vec3 pp=p-hpos;
    pp=lookat(point)*pp;
    pp.y-=abs(sin(animation_time))*3.+(fparam-(2.-fparam));
    pp.yz*=rot(-animation_time);
...
vec3 march(vec3 from, vec3 dir) {
    ...
    if (d<.001) {
        ...
        col+=step(.3,hp)*step(.9,fract(pr.z*.05+animation_time*.5+hp*.1))*.7;
        col+=step(.3,hexpos)*step(.9,fract(zz*.05+animation_time+hexpos*.1))*.3;
    }
    col+=g*.03;
	col.rb*=rot(odir.y*.5);
	return col;
...
void main()
{
    vec2 uv = VERTEX_COLOR.xy; - 0.5;
    float t=animation_time*2.;
    vec3 from=path(t);
    if (mod(animation_time-10.,20.)>10.) {
    ...
    point.xz*=rot(sin(animation_time)*.2);
    dir=lookat(dd)*dir;
...
export default special_frag_shader
