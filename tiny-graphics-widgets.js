...
const Controls_Widget = (widgets.Controls_Widget = class Controls_Widget {
	...
	constructor(element, scenes) {
		const rules = [
			...
			'.controls-widget table.control-box td .control-div { background: White; height:500px; padding: 5px 5px 5px 30px; }',
	...
	render(time = 0) {
		...
		for (let panel of this.panels)
			for (let live_string of panel.querySelectorAll('.live_string'))
...
const Code_Widget = (widgets.Code_Widget = class Code_Widget {
	...
	format_code(code_string) {
		...
		for (let t of new Code_Manager(code_string).tokens)
			if (
				t.type == 'name' &&
				[...Object.keys(tiny), ...Object.keys(this.definitions)].includes(
					t.value
				)
			) {
...
const Editor_Widget = (widgets.Editor_Widget = class Editor_Widget {
	constructor(element, initially_selected_class, canvas_widget, options = {}) {
		...
		if (initially_selected_class) this.select_class(initially_selected_class)
	}
